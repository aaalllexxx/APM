...
> routers: auto | dict[route: screen name] - Роуты приложения, в случае если выбран автоматический режим, все файлы в screen_path будут определяться как view, и класс экрана внутри должен содержать параметр route, в котором должен быть указан путь до экрана на сервере